...
/**
 * @author squizlabs
 * @fork https://github.com/squizlabs/PHP_CodeSniffer/blob/master/src/Standards/PEAR/Sniffs/Functions/FunctionCallSignatureSniff.php
 */

declare(strict_types=1);

namespace BaBeuloula\PhpCS\Sniffs\Functions;
...
class FunctionCallSignatureSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if (T_CLOSE_CURLY_BRACKET === $tokens[$stackPtr]['code']
            && true === \array_key_exists('scope_condition', $tokens[$stackPtr])
        ) {
            // Not a function call.
            return;
        }
        ...
        if (false === \array_key_exists('parenthesis_closer', $tokens[$openBracket])) {
            // Not a function call.
            return;
        }
     ...
     * @return bool
     */
    public function isMultiLineCall($openBracket, $tokens)
    {
    ...
    public function processSingleLineCall(File $phpcsFile, $stackPtr, $openBracket, $tokens)
    {
        ...
        } elseif (0 < $requiredSpacesAfterOpen) {
            $spaceAfterOpen = 0;
    ...
    public function processMultiLineCall(File $phpcsFile, $stackPtr, $openBracket, $tokens)
    {
        ...
        for ($i; $i < $closeBracket; $i++) {
            ...
            if ($tokens[$i]['line'] !== $lastLine) {
                ...
                if (true === \array_key_exists($tokens[$i]['code'], Tokens::$heredocTokens)) {
                    continue;
                }

                // Ignore multi-line string indentation.
                if (true === \array_key_exists($tokens[$i]['code'], Tokens::$stringTokens)
